...
const Book = async function (req, res){
    ...
    try {
       ...
       /*----------------------------validations ----------------------------*/
        ...
        if (!Validator.isValidString(title)) return res.status(400).send({ status: false, message: "Title Is not valid" })
        ...
        if (!Validator.isValidString(excerpt)) return res.status(400).send({ status: false, message: "Excerpt is not vaild" });
